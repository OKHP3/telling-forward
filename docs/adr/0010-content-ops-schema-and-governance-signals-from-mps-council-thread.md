...
> **Numbering note (updated 2026-08-19, post-lock-clear):** this ADR was first drafted as `0009-capsule-provenance-and-term-ledger-field-schemas.md`, then renumbered to 0010 when another concurrently-active Cowork session independently claimed 0009 for a different ADR at the same time. That earlier draft's content is fully carried forward here (items 1 and 2 below) and the draft itself has since been moved to `docs/adr/_to_delete/`. Since this ADR's own numbering conflict was written up, the *other* session resolved its side of the collision on its own — the competing file that once raced for this same number is now committed as `0011-provenance-and-process-artifact-concepts-from-first-mover-claim.md`, and a separate `0009-transformation-fidelity-and-readability-assist-concepts.md` (unrelated content, from yet another session) now sits cleanly at 0009. As of this update, `0010` is uncontested in git history. One collision remains open in committed history and is untouched by this ADR: two different files are both committed as `0007` (`0007-reader-accessibility-register-and-clarity-pass-from-bac-thread.md` and `0007-scene-purpose-framing-from-synopsis-discipline.md`). Left for the owner to consolidate.
