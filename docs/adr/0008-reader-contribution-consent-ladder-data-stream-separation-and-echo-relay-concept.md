...
This item does not need new schema of its own. ADR-0005 item 2 already proposes a side-car provenance layer on capsules/scenes (which ingestion tier or engine produced or matured a capsule, a human-edited-after flag); a GitHub-native, commit/PR/Issue-referencing field is the natural way to satisfy the "timestamped record" half of this concept once that layer and the `capsules` table exist. A more detailed field-schema ADR existed briefly during this repository's 2026-08-19 multi-session reconciliation but was reworked by another session into a different shape; rather than cite an unstable filename, this item rides on ADR-0005 item 2 directly, which has stayed stable throughout the day's numbering churn.
...
`docs/adr/` went through a live, multi-session numbering race for most of 2026-08-19, traced to more than one process running git operations against the same working directory at once — a recurring `.git/index.lock` contention, manually cleared by the project owner. Most of the pileup has since been resolved and pushed to `origin/main` by another process's batch commit (`a485309`). As of this ADR's latest revision, the only unresolved collision left in `docs/adr/` is two files sharing number 0007 (`0007-reader-accessibility-register-and-clarity-pass-from-bac-thread.md`, referenced by open question 15.13, and `0007-scene-purpose-framing-from-synopsis-discipline.md`) — unrelated to this ADR's own content and out of scope for this thread to resolve. This ADR was never renumbered and stayed at 0008 throughout. Whether this file and `docs/decisions/open-questions.md`'s 15.14/15.15 rows are already part of the pushed commit or still need `git add`/`git commit` could not be confirmed from this session (no git-status access to the project owner's working directory) — the project owner should check `git status` directly before assuming either way.
