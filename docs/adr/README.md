...
| 0012 | Accepted (2026-09-24); design intent, not implemented | Scene-purpose framing (why it exists / what it advances / how it hands off) accepted as design intent for Concept Board's Promote-to-scene action; the GitHub boundary reference is separately accepted in ADR-0003 and clarified by ADR-0013 | Carry the three-field purpose-note shape into Concept Board's data model when that implementation is scoped; no action outstanding until then |
