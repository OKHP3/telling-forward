...
**Update, post-commit:** this ADR is now committed at `docs/adr/0009-transformation-fidelity-and-readability-assist-concepts.md`, and a concurrent session's own cleanup pass has since resolved most of the wider pileup independently: the two files that briefly collided at 0010 are now uniquely numbered 0010 and 0011. As of this update, only one collision remains in `docs/adr/`: two files still share number 0007 (`0007-reader-accessibility-register-and-clarity-pass-from-bac-thread.md` and `0007-scene-purpose-framing-from-synopsis-discipline.md`). That is the only outstanding numbering item for the project owner to resolve; this ADR's own number is stable.
